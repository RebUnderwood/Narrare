...
```godot
Narrare.say_someting("Hello world!");
```
...
> The player's inventory. Contains a PackedStringArray of names of interactables which reference interactables in the Interactables.gd script's `objs` variable. To add an interactable to the player's inventory, use `Narrare.add_to_inventory(item_name: String)`. To remove an item, use `remove_from_inventory(item_name: String)`.
